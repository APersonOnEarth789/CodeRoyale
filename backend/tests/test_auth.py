...
def test_signup_success(client):
    signup_response, signup_json = signup_user(client)
...
def test_signup_duplicate(client):
    signup_user(client)
    signup_response, _ = signup_user(client)
...
def test_signup_username_too_short(client):
    signup_response, _ = signup_user(client, username="a")
...
def test_signup_username_too_long(client):
    signup_response, _ = signup_user(client, username="a"*129)
    assert_validation_error(signup_response)
...
def test_signup_password_too_short(client):
    signup_response, _ = signup_user(client, password="a")
    assert_validation_error(signup_response)
...
def test_signup_password_too_long(client):
    signup_response, _ = signup_user(client, password="a"*257)
    assert_validation_error(signup_response)
...
def test_login_success(client):
    signup_user(client)
    login_response, login_json = login_user(client)
...
def test_login_with_wrong_username(client):
    signup_user(client)
    login_response, _ = login_user(client, username="Lemon")
...
def test_login_with_wrong_password(client):
    signup_user(client)
    login_response, _ = login_user(client, password="Lime12345")
    assert login_response.status_code == 401
    assert "Incorrect username or password" in login_response.text
